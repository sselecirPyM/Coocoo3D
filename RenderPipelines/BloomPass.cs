...
""", "csmain");
    ...
    public void Execute(RenderPipelineView renderPipelineView)
    {
        ...
        var addr1 = graphicsContext.UploadCBV<float>(cbv1);


        graphicsContext.SetPSO(shader_bloom.Get(Keyword_Bloom.BLOOM_1));
        ...
        graphicsContext.Dispatch((intermediaTexture.width + 63) / 64, intermediaTexture.height, 1);

        cbv1[0] = 0;
        cbv1[1] = 1.0f / intermediaSize.Y;
        cbv1[2] = 1.0f / (float)output.width;
        cbv1[3] = 1.0f / (float)output.height;
        var addr2 = graphicsContext.UploadCBV<float>(cbv1);


        graphicsContext.SetPSO(shader_bloom.Get(Keyword_Bloom.BLOOM_2));
        ...
        graphicsContext.Dispatch(output.width, (output.height + 63) / 64, 1);
    }
